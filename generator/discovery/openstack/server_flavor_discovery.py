...
class ServerFlavorDiscovery:
    ...
    def _get_cpu(self, os_flavor: OS_Flavor) -> CPU:
        ...
        if os_flavor.name.startswith("SCS-"):
            ...
            if len(parts) >= 2:
                ...
                cpu.numberOfCores = os_flavor.vcpus
        return cpu
    ...
    def _get_disks(self, os_flavor: OS_Flavor) -> List[Disk]:
        ...
        if os_flavor.name.startswith("SCS-"):
            # parse SCS flavor name
            parts = os_flavor.name.split("-")[3]
            if len(parts) < 4:
                # either no SCS compliant flavor name or disk spec missing
                disks.append(Disk(diskSize=MemorySize(value=os_flavor.disk, unit=const.UNIT_GB)))
            else:
                ...
                for i in range(int(number) - 1):
                    disks.append(Disk(diskSize=MemorySize(value=size, unit=const.UNIT_GB), diskType=disk_type,
                                      diskBusType=disk_bus_types))
        return disks

    def _parse_optional_flavor_properties(self, os_flavor: OS_Flavor, gx_flavor: GX_Flavor):
        ...
        for suffix in parts:
            ...
            # parse hypervisor
                ...
                gx_flavor.hypervisor = Hypervisor(hypervisorType="KVM",
                                                  copyrightOwnedBy=common.get_copyright_owner(self.config, const.CONFIG_HV_KVM),
                                                  license=common.get_license(self.config, const.CONFIG_HV_KVM),
                                                  resourcePolicy=common.get_resource_policy(self.config, const.CONFIG_HV_KVM))
            elif suffix == "xen":
                gx_flavor.hypervisor = Hypervisor(hypervisorType="KVM",
                                                  copyrightOwnedBy=common.get_copyright_owner(self.config, const.CONFIG_HV_XEN),
                                                  license=common.get_license(self.config, const.CONFIG_HV_XEN),
                                                  resourcePolicy=common.get_resource_policy(self.config, const.CONFIG_HV_XEN))
            elif suffix == "vmw":
                gx_flavor.hypervisor = Hypervisor(hypervisorType="KVM",
                                                  copyrightOwnedBy=common.get_copyright_owner(self.config, const.CONFIG_HV_VMW),
                                                  license=common.get_license(self.config, const.CONFIG_HV_VMW),
                                                  resourcePolicy=common.get_resource_policy(self.config, const.CONFIG_HV_VMW))
            elif suffix == "hyv":
                gx_flavor.hypervisor = Hypervisor(hypervisorType="KVM",
                                                  copyrightOwnedBy=common.get_copyright_owner(self.config, const.CONFIG_HV_HYV),
                                                  license=common.get_license(self.config, const.CONFIG_HV_HYV),
                                                  resourcePolicy=common.get_resource_policy(self.config, const.CONFIG_HV_HYV))
            ...
            # parse CPU architecture details
                ...
                # Intel x86-64
                gx_flavor.cpu.cpuArchitecture = "x86-64"
                ...
                #  	ARM v8+
                gx_flavor.cpu.cpuArchitecture = "AArch-64"
                gx_flavor.cpu.vendor = "AMD"
                ...
                #  	ARM v8+
                gx_flavor.cpu.cpuArchitecture = "RISC-V"
            # parse frequency
            elif suffix.endswith("hhh"):
    ...
    def _get_disk_caps(self, disk_caps: str) -> Tuple[int, int, str, str]:
        ...
        return int(number), int(size), disk_type, disk_bus_types
